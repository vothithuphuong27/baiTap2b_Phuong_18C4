...
def main_ex_2_b():
    data = generate_dataset(ex_2_dataset)
    real_label = []  # nhãn thực tế
    predicted_label = []  # nhãn dự đoán
    for test in data.get("testing"):
        real_label.append(test.label)  # thêm nhãn thực tế vào mảng
        trained_data = training(data.get("training"), test)  # huấn luyện
        # lấy 5 láng giềng gần nhất
        knn = get_k_nearest_neighbors(trained_data, k=5)
        label = get_predicted_label(knn)  # lấy nhãn dự đoán
        predicted_label.append(label)  # thêm nhãn dự đoán vào mảng
    cm = np.zeros((2, 2), dtype=int)  # khởi tạo mảng
